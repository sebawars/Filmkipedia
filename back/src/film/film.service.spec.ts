...
describe("FilmService Tests", (): void => {
  ...
  test("Find all films, returns existing films.", async (done: Function): Promise<
    void
  > => {
  ...
  test("FindById, with existing id, returns existing film.", async (done: Function): Promise<
    void
  > => {
    const idToFind = 1;
    ...
    const retrievedFilm: Film = await filmService.findById(idToFind);

    expect(retrievedFilm).not.toBeNull();
    expect(retrievedFilm).toEqual(existingFilm1);

    done();
  });

  test("FindById, with nonexisting id, returns undefined.", async (done: Function): Promise<
    void
  > => {
    const idToFind = 1;
    ...
    expect(retrievedFilm).not.toBeUndefined();

    done();
  });

  test("Save, with invalid film, throws BadRequestException.", async (done: Function): Promise<
    void
  > => {
    const newFilm: Film = new FilmBuilder().build();

    expect(filmService.save(newFilm)).rejects.toBeInstanceOf(
      BadRequestException
    );
  ...
  test("Save, with valid film, creates the film.", async (done: Function): Promise<
    void
  > => {
  ...
  test("Save, with valid existing film, updates the film.", async (done: Function): Promise<
    void
  > => {
  ...
  test("Delete existing film, deletes the film.", async (done: Function): Promise<
    void
  > => {
